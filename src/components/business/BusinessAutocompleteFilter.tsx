...
interface BusinessAutocompleteFilterProps {
  field: string;
  placeholder: string;
  selected: string[];
...
export function BusinessAutocompleteFilter({
  field,
  placeholder,
  selected,
  onChange,
  disabled,
}: BusinessAutocompleteFilterProps) {
  ...
  useEffect(() => {
    ...
    const cacheKey = `biz:${field}:${query}`;
    ...
    timer.current = setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          `/api/businesses/autocomplete?field=${field}&query=${encodeURIComponent(query)}`,
  ...
  return (
    <div className="mt-1">
      <div className="relative">
        ...
        />
        {loading && query.length > 0 && !disabled && (
